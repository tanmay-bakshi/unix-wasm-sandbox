...
impl WasiFs {
    ...
    pub fn swap_file(
        ...
        match fd {
            __WASI_STDIN_FILENO => {
                self.set_fd_filetype(fd, Filetype::RegularFile);
    ...
    fn set_fd_filetype(&self, fd: WasiFd, filetype: Filetype) {
        let Ok(inode) = self.get_fd_inode(fd) else {
            return;
        };
        let Ok(mut stat) = inode.stat.write() else {
            return;
        };
